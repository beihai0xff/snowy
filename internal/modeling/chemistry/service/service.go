...
func (s *defaultService) AnalyzeReaction(_ context.Context, raw string) (*chemistry.ChemistryReactionPackage, error) {
	...
	if rtype == chemistry.ReactionRedox || rtype == chemistry.ReactionElectrolysis ||
		rtype == chemistry.ReactionCombustion {
...
//nolint:cyclop // Electron-transfer inference is a small heuristic table over reactants/products.
func deriveElectronTransfer(eq *balancer.Equation) []chemistry.ETransfer {
	...
	isMetal := func(e string) bool {
		return slices.Contains(metals, e)
	}
