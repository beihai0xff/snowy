...
> 历史设计说明：本文保留 v5 阶段对引用、证据和学习档案的产品设想；其中真实资料索引与可核验引用不代表当前默认问答链路已经接入。当前默认问答链路为 LLM 直答 + runtime grounding。
...
- LLM 失败或生成结果未通过校验时不再生成规则/模板兜底结果，接口透出明确失败原因；
