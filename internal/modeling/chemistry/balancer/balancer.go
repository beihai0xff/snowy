...
func parseSide(s string) ([]SpeciesCount, error) {
	...
	for _, p := range parts {
		...
		if err != nil {
			return nil, fmt.Errorf("%w: %s: %w", ErrParse, formula, err)
		}
...
//
//nolint:cyclop,gocognit,gocyclo // Formula parsing is a compact recursive descent over chemistry notation.
...
//
//nolint:cyclop // Balancing keeps matrix construction and coefficient projection in one routine.
...
//
//nolint:cyclop,funlen,gocognit // RREF implementation is intentionally local to avoid hiding algebra state.
