...
http_post_json() {
  ...
  curl -fsS --max-time 240 \
    ...
    -d "$data" \
    "$url" >/tmp/snowy-smoke-response.json
  python3 - <<'PY'
import json, sys
...
if '大模型直答' not in tags or 'mimo' not in tags:
    print({'error':'search did not use MiMo direct LLM answer','tags':tags,'answer':answer[:240]}, file=sys.stderr)
    sys.exit(1)
if 'api key is empty' in answer or '诊断信息' in answer or '当前大模型知识问答暂时不可用' in answer:
    print({'error':'search returned fallback diagnostic','answer':answer[:500]}, file=sys.stderr)
    sys.exit(1)
PY
}
...
for path in / /search /physics /biology /learning; do
...
http_post_json "$WEB_BASE/api/v1/agent/chat" '{"message":"牛顿第二定律是什么","mode":"search","filters":{"subject":"physics"}}' || fail "agent chat failed"
ok "core APIs are OK"
