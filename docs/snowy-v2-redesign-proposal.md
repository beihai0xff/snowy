...
当前 Snowy 已具备面向高中生的 AIGC 学习平台雏形：统一知识检索、物理 / 3D 场景代码生成、生物结构化建模，以及基于 Go + Gin + Eino + OpenSearch 的工程基座。v2 的核心建议不是简单增加功能，而是将产品从“AI 问答 + 代码生成 + 图谱输出”的组合，重构为一个统一的 **AI 科学建模与交互仿真实验室**。

新版 Snowy v2 建议围绕一条稳定学习链路组织能力：

```text
提问 → 检索证据 → 构建模型 → 交互仿真/可视化 → 反思归纳 → 练习验证
```

对应产品定位升级为：

> Snowy v2 是面向高中生的 AIGC 知识检索、物理仿真与生物可视化统一建模工具。它以课标与可信资料为依据，以大模型推理生成的结构化模型为中间层，以可交互仿真和可视化为学习载体，帮助学生把“知识点、题目条件、变量关系、过程变化和解题迁移”连接起来。

关键重构方向：

1. **知识检索升级为证据型学习搜索**：从“返回答案”升级为“基于引用、知识图谱、易错点、题型映射和下一步建议的学习入口”。
...
| 方向 | 当前能力 | v2 可继承价值 |
|---|---|---|
| 知识检索 | 统一索引课本、考纲、题库、讲义，基于 RAG 输出答案与引用 | 可作为 v2 的证据层与学习入口 |
...
当前信息架构将知识检索、物理建模、生物建模拆成独立入口，有利于 MVP 快速验证，但对高中生而言，真实学习路径往往不是按功能模块切换，而是围绕一个问题不断深入：
...
1. **主产物是生成式模型包**：大模型基于引用证据和题意，输出公式、变量、过程、关系、交互任务、反馈规则和评估题。
...
**Snowy v2 是一个面向高中生的 AI 科学建模与交互仿真实验室，将可信知识检索、物理仿真、生物可视化和学习验证整合到同一学习工作台。**
...
v2 建议新增统一学习工作台，作为知识检索、物理仿真、生物可视化的共同承载界面。
...
将知识检索从“问答入口”升级为“学习导航入口”。学生不只获得答案，还能知道：
...
- **LLM Reasoning First**：检索证据后，由大模型生成完整建模包，具体公式、变量关系、过程阶段、挑战目标和反馈文案尽可能来自模型推理。
...
├── EvidenceRef[]              # 引用证据与 grounding 片段
...
| Generative Model Compiler | 调用大模型将检索证据和领域解析结果编译为生成式模型包 | Modeling Core |
...
1. **意图识别**：判断是知识检索、物理仿真、生物可视化、实验题、复盘还是混合任务。
...
- 建立物理、生物、知识检索的 schema 校验、领域校验和低可信回退标准。
- 输出交互原型和核心链路验收标准。

### M1：证据型学习搜索 + 建模 grounding（2~3 周）

- 在现有 RAG 检索结果上新增知识标签、易错点、题型映射、下一步建议。
